...
Claude Code CLI 的构建与部分运行时特性**深度依赖 Bun**。但需要注意，这并不意味着它是一个 Bun-only 项目 —— 源码中明确检测 Node 版本（`setup.ts:69-79`），并提供了 `isRunningWithBun()` 函数来区分运行态（`utils/bundledMode.ts:1-22`），说明项目同时考虑了非 Bun 环境的兼容。
