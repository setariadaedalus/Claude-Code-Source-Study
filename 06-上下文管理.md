...
> 本篇是《深入 Claude Code CLI 源码》系列的第 6 篇。我们将深入分析 Claude Code 如何在有限的 context window 内支撑无限长度的对话——从 token 预算管理、多级压缩策略，到文件状态缓存与 compact 后恢复的设计。
>
> **范围说明**：上下文的"构建"（System Prompt 组装、CLAUDE.md 注入、git status 获取）已在第 4 篇和第 7 篇中分别覆盖。本篇聚焦于上下文构建完成后的**压缩、清理与恢复**——即当 context window 不够用时，Claude Code 如何在不中断对话的情况下释放空间并重建必要的工作上下文。
...
Claude Code 的解决方案是一套**多层次的上下文管理体系**——从最轻量的 Microcompact（清理工具结果）到最重量级的 Full Compact（用模型总结整个对话），形成了一个完整的上下文压力梯度响应系统。
...
值得注意的是，Warning 和 Error 阈值在当前源码中实际相同（都是 20K 缓冲），但它们驱动不同的 UI 表现——Warning 是黄色提示，Error 是红色提示。分开定义为未来独立调整留了空间。
...
### 2.3 三种 Microcompact 路径

源码中实际存在三种 Microcompact 实现路径，按优先级选择：

**路径 1：Time-based Microcompact**

**文件**：`services/compact/microCompact.ts:401-530`

当用户离开一段时间后回来（gap > 阈值），服务端的 prompt cache 已经过期，此时无需保护缓存，直接替换旧的工具结果内容：
...
这种方式是**直接修改消息内容**（mutate），因为 cache 已经冷了，没有需要保护的前缀。

**路径 2：Cached Microcompact（cache_edits API）**

**文件**：`services/compact/microCompact.ts:305-399`

当 prompt cache 仍然温热时，不能直接修改消息内容（那会破坏缓存），而是通过 Anthropic API 的 `cache_edits` 机制，在 API 层面删除指定工具的 `tool_result`：
...
**路径 3：API-level Context Management**

**文件**：`services/compact/apiMicrocompact.ts`

最新的实验性路径，使用 Anthropic API 原生的 context management 策略（`clear_tool_uses_20250919`、`clear_thinking_20251015`），将上下文清理完全委托给 API 层：
...
## 四、FileStateCache — 文件状态的 LRU 缓存

**文件**：`utils/fileStateCache.ts`

FileStateCache 不直接参与 compact，但它是上下文管理中不可或缺的一环——它记录了模型"见过"哪些文件的哪些版本，避免重复读取。
...
1. **双重限制**：`max` 限制条目数（默认 100），`maxSize` 限制总大小（默认 25MB）。这防止大量大文件导致内存膨胀。

2. **路径标准化**：所有 key 在存取时都经过 `normalize()`，确保 `/foo/../bar` 和 `/bar` 命中同一条缓存。

3. **`isPartialView` 标记**：当文件内容是被截断注入的（如 CLAUDE.md 去掉 HTML 注释），这个标记告诉 Edit/Write 工具必须先做一次完整的 Read，不能基于缓存的部分内容进行编辑。

4. **Compact 时清理**：Full Compact 后，`readFileState.clear()` 清除所有缓存，然后通过 `createPostCompactFileAttachments` 选择性恢复最重要的文件。
...
[第 7 篇：Prompt Cache — 让每次 API 调用都省钱的缓存策略](./07-Prompt-Cache.md)

我们将深入 `CacheSafeParams`、`splitSysPromptPrefix()` 和 `SYSTEM_PROMPT_DYNAMIC_BOUNDARY`，揭示 Claude Code 如何通过精巧的 prompt 结构设计最大化 Anthropic API 的 prompt cache 命中率，让每次 API 调用都尽可能少花钱。
